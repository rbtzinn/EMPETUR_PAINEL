      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-10">
        <FadeIn className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto shrink-0">
            <button onClick={() => setFiltros({ municipio: "", ciclo: "", ano: "", artista: "", dataEvento: "" })} className="w-full sm:w-auto text-[#00AEEF] font-bold text-sm uppercase tracking-wider hover:underline bg-blue-50 px-5 py-3 rounded-xl transition-colors">
              ↺ Limpar Filtros
            </button>
                  ...
                  {filtrados.length > 0 ? (
                    filtrados.map((d) => (
                      <tr key={d.id} className="hover:bg-blue-50/40 transition-colors border-b border-slate-100 last:border-0 group">
                        <td className="p-6 text-slate-500 font-mono text-[10px] font-bold">{d.numeroEmpenho}</td>
                        ...
                        >
                          <span className="inline-block whitespace-nowrap bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-xs font-bold hover:bg-slate-200 transition-colors">
                            {d.ciclo}
                          </span>
                        </td>

                        <td className="p-6 text-sm text-slate-600">{d.dataEvento}</td>
                        <td className="p-6 text-base font-mono font-black text-[#00AEEF] text-right">{Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(Number(d.valor) || 0)}</td>
                      </tr>
                    ))
                  ) : (
                    <tr><td colSpan="6" className="p-12 text-center text-slate-400 font-medium">Nenhum registro encontrado.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
