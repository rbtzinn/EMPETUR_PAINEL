...
export default function GlossarySection() {
  ...
  return (
    <section id="glossario" className="py-24 bg-white" aria-label="Glossário e Dicionário de Dados">
      <div className="max-w-4xl mx-auto px-6">
            ...
            return (
              <FadeIn key={i} delay={i * 0.1}>
                <div className={`border border-slate-100 rounded-2xl overflow-hidden transition-all duration-300 ${isOpen ? 'bg-slate-50 shadow-xl shadow-blue-900/5 hc-card' : 'bg-white hover:bg-slate-50 hc-card'}`}>
                  <button onClick={() => setOpenIndex(isOpen ? null : i)} className="w-full flex items-center justify-between p-6 text-left cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#00AEEF]">
        ...
        {/* 🔴 BLOCO DE DICIONÁRIO DE DADOS (Exigência do Selo Diamante) */}
