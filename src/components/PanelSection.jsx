...
import DropdownPesquisavel from "./DropdownPesquisavel";
...
export default function PanelSection({ id, csvUrl, lookerShareUrl }) {
  ...
  const getOpcoesCascata = (campoCorrente) => {
    const dadosPossiveis = dados.filter((d) => {
      return (
        (filtros.municipio === "" ||
          campoCorrente === "municipio" ||
          d.municipio === filtros.municipio) &&
        (filtros.ciclo === "" ||
          campoCorrente === "ciclo" ||
          d.ciclo === filtros.ciclo) &&
        (filtros.ano === "" ||
          campoCorrente === "ano" ||
          d.ano === filtros.ano) &&
        (filtros.artista === "" ||
          campoCorrente === "artista" ||
          d.artista === filtros.artista) &&
        (filtros.dataEvento === "" ||
          campoCorrente === "dataEvento" ||
          d.dataEvento === filtros.dataEvento)
      );
    });
  ...
  const handleAcessarPainel = (e) => {
    e.preventDefault();
    e.stopPropagation();
    window.open(lookerShareUrl, "_self");
  };

  if (loading) {
    return (
      <div className="py-32 flex justify-center bg-[#F8FAFC]">
        <Text className="font-bold text-[#0B2341] animate-pulse text-xl">
          Sincronizando base eFisco...
        </Text>
      </div>
    );
  }
    ...
    <section id={id} className="py-24 bg-[#F8FAFC]">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-10">
        <FadeIn className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <h2 className="text-3xl md:text-5xl font-black text-[#0B2341] tracking-tight mb-2">
              Consulta Rápida
            </h2>
          ...
          <Card className="rounded-[2rem] border-none shadow-xl shadow-blue-900/5 bg-white p-6 mb-8 overflow-visible">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <DropdownPesquisavel
                ...
                options={getOpcoesCascata("municipio")}
              />
              <DropdownPesquisavel
                label="Todos os Ciclos"
                value={filtros.ciclo}
                onChange={(v) => setFiltros({ ...filtros, ciclo: v })}
                options={getOpcoesCascata("ciclo")}
              />
              <DropdownPesquisavel
                label="Todos os Anos"
                value={filtros.ano}
                onChange={(v) => setFiltros({ ...filtros, ano: v })}
                options={getOpcoesCascata("ano")}
              />
              <DropdownPesquisavel
                label="Todos os Artistas"
                value={filtros.artista}
                onChange={(v) => setFiltros({ ...filtros, artista: v })}
                options={getOpcoesCascata("artista")}
              />
              <DropdownPesquisavel
                label="Todas as Datas"
                value={filtros.dataEvento}
                onChange={(v) => setFiltros({ ...filtros, dataEvento: v })}
                options={getOpcoesCascata("dataEvento")}
              />
            </div>
          </Card>
        </FadeIn>
        ...
        <FadeIn delay={0.2}>
          <Card className="rounded-[2rem] border-none shadow-2xl shadow-blue-900/5 bg-white overflow-hidden p-0">
            <div className="p-6 md:p-8 border-b border-slate-50 bg-[#0B2341] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <Text className="font-bold text-white text-sm tracking-wide">
                DETALHAMENTO OFICIAL
              </Text>
              <div className="flex items-center gap-4">
                <Badge className="font-bold text-white text-sm tracking-wide">
            ...
            <div className="h-[600px] overflow-y-auto scrollbar-moderna">
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead className="bg-slate-50/95 backdrop-blur-sm sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-6 text-xs font-black text-[#0B2341] uppercase tracking-wider whitespace-nowrap">
                      Nº Empenho
                    </th>
                    <th className="p-6 text-xs font-black text-[#0B2341] uppercase tracking-wider whitespace-nowrap">
                      Artista
                    ...
                    <th className="p-6 text-xs font-black text-[#0B2341] uppercase tracking-wider text-right whitespace-nowrap">
                      Valor Pago
                    </th>
                  </tr>
                </thead>
                  ...
                  {filtrados.length > 0 ? (
                    filtrados.map((d) => (
                      ...
                      >
                        ...
                        >
                          <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-xs font-bold hover:bg-slate-200 transition-colors">
                            {d.ciclo}
                          </span>
                        </td>
