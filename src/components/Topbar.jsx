...
export default function Topbar({ lookerShareUrl }) {
  ...
  const handleAcessarPainel = () => {
    window.location.href = lookerShareUrl;
  };
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[#00AEEF] font-black text-[8px] md:text-[9px] uppercase tracking-[0.2em]">
                Pernambuco
              </span>
            </div>
          </div>
            ...
            <nav className="hidden lg:flex gap-8">
              {['painel', 'gestao', 'sobre', 'glossario'].map((id) => (
                ...
                >
                  ...
                  {activeSection === id && (
                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-[#00AEEF] rounded-full" />
                  )}
                </a>
              ))}
            </nav>
            ...
            {['painel', 'gestao', 'sobre', 'glossario'].map((id) => (
              <a
                ...
                className={`text-lg font-bold uppercase tracking-widest ${
                  activeSection === id ? 'text-[#00AEEF]' : 'text-white'
                }`}
              >
                {id === 'gestao' ? 'Gestão' : id.charAt(0).toUpperCase() + id.slice(1)}
              </a>
